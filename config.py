...
BUILTIN_PROFILES: Dict[str, CleanupProfile] = {
    ...
    "standard": CleanupProfile(
        ...
        rules=[
            ...
            "teams_apps", "service_profiles_temp",
        ],
    ),
    "deep": CleanupProfile(
        name="deep",
        description="Deep system cleanup (standard + WinSxS, old drivers, orphaned installers)",
        rules=[
            "temp_files", "windows_update", "prefetch", "caches",
            "logs_reports", "delivery_optimization", "installer",
            "old_windows", "recycle_bin", "shader_cache", "icon_cache",
            "teams_apps", "service_profiles_temp", "orphaned_installers",
            "winsxs_cleanup", "driver_store_cleanup",
        ],
    ),
    ...
    "everything": CleanupProfile(
        ...
        rules=[
            ...
            "teams_apps", "service_profiles_temp", "orphaned_installers",
            "winsxs_cleanup", "driver_store_cleanup",
            "dev_nodejs", "dev_python", "dev_dotnet",
            "dev_java", "dev_rust_go", "dev_docker", "dev_ide",
            "registry",
        ],
    ),
